...
Inductive instrR {S} A B (f : A -> S -> Exc B * S -> Prop)
  : A -> (S * list (A * B)) -> Exc B * (S * list (A * B)) -> Prop :=
  | instrRvalue :
      forall a s l b s1,
        f a s (value b,s1) ->
        instrR f a (s,l) (value b, (s1, l ++ [(a,b)]))
  | instrRerror :
      forall a s l s1,
        f a s (error,s1) ->
        instrR f a (s,l) (error, (s1, l)).

Lemma instrR_value_inv S A B (f : A -> S -> Exc B * S -> Prop)
                       a s l b s1 l1 :
  instrR f a (s,l) (value b, (s1,l1)) ->
  f a s (value b,s1) /\ l1 = l ++ [(a,b)].
Proof. intros H; now inversion H; subst; auto. Qed.

Lemma instrR_error_inv S A B (f : A -> S -> Exc B * S -> Prop)
                       a s l s1 l1 :
  instrR f a (s,l) (error, (s1,l1)) ->
  f a s (error,s1) /\ l1 = l.
Proof. intros H; now inversion H; subst; auto. Qed.
...
with Wrap_Eval_x :
  ...
  Exc D.t * (state' * list (Var.t * D.t)) -> Prop :=
  | Wrap_Eval_x0 :
...
with Eval_rhs :
  Var.t ->
  state' -> Exc D.t * (state' * list (Var.t * D.t)) -> Prop :=
  | Eval_rhs0 :
    forall x f s dsl0,
      EvalGet_x x f ->
...
- idtac. intros x f.
  ...
  + intros [s l] [d [s0 l0] ] Hevalx Ievalx fi Hfi.
    ...
    eapply wrap_rel_ErrorT_State_fun;
      [| now refine Hfi | now refine Hfi'].
    clear - Hfun.
  ...
  + intros [s l] [d [s1 l1] ] Hevalx Ievalx fi Hfi.
    ...
    eapply wrap_rel_ErrorT_State_fun; [| refine Hfi | refine Hfi'].
    clear - Hff'.
    intros y [s l] [d [s0 l0] ] [d' [s0' l0'] ].
    unfold fi, fi'. intros Hfi Hfi'.
    inversion Hfi; subst; inversion Hfi'; subst;
...
(* Solve 1 *)
- idtac.
  intros x s s1 l1 Hnstaxs s0 Heval Ieval s1' Hsol'.
  inversion Hsol' as [| ? ? ? ? ? ? Heval' | ? ? ? ? ? ? ? Heval' | ? ? ? ? ? ? ? ? ? ? Heval' ].
  + now subst s1'.
...
Qed.
Local Hint Resolve Inv_1_trans.
...
Local Hint Resolve Inv_sigma_trans.
...
Qed.
Local Hint Resolve Inv_sigma_infl_trans.
...
Definition Inv_EvalGet
  (x y : Var.t) (s : state') (p : Exc D.t * state') :=
  let (od,s') := p in
  Inv_0 s ->
  Inv_corr s ->
  Inv_0 s' /\
...
Definition Inv_EvalGet_x
  (x : Var.t) (f : Var.t -> state' -> Exc D.t * state' -> Prop) :=
  forall y s ds1,
...
Definition Inv_Wrap_Eval_x
  ...
  (dsl' : Exc D.t * (state' * list (Var.t * D.t))) :=
  let (s, ps) := sl in
  let '(od, (s', ps')) := dsl' in
  is_called x s ->
  Inv_0 s ->
...
(* EvalGet1 *)
- idtac. intros x y s s0.
  destruct_pose_state s.
  destruct_pose_state s0.
  intros H Hinv s1 d.
  intros Hxcal I0s Icorrs.
  assert (Hval : getval s1 = getval s0) by auto.
  rewrite Hval; clear Hval.
  unfold Inv_Solve in Hinv.
  destruct Hinv as [I0 [I1 [Isig [Isiginfl [Icor (*[Hstaby*) [Hnstaby Hmon] (*]*) ] ] ] ] ]; auto.
  split; [| split; [| split; [| split; [| split; [| split; [| split; [| split] ] ] ] ] ] ]; auto; try easy.
  + intros d0 e; inversion e; subst d0; clear e; now auto.
  + clear - Hnstaby I1.
    case (is_stable_dec y s) as [i | n];
      simpl in *; now intuition; fsetdec.
  + now apply Inv_sigma_infl_lemma1.
...
- idtac.
  ...
  - intros [s ps] [od [s0 ps'] ] Evalgetx Ievalgetx f' Hf'.
    (*assert (Htmp := wrap_rel_ErrorT_State_Ans_inv Hf').*)
    inversion_clear Hf'; subst.
    destruct_pose_state s.
    destruct_pose_state s0.
    split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split] ] ] ] ] ] ] ] ]; auto; try easy.
    + now firstorder.
    + intros d e; inversion e; subst d; clear e; easy.
    + intros d e; inversion e; subst d; clear e.
      intros Hval Hps'0.
      assert (Hps' : ps' = deps (rhs x) (getval s0))
        by (eapply valid_legal_subtree_Ans; eauto).
      unfold Inv_corr_var; rewrite <- Hps'.
      now firstorder.
  - intros [s ps] [od [s1 ps'] ] Evalgetx Ievalgetx f' Hf'.
    case od as [d |].
    + assert (Htmp := wrap_rel_ErrorT_State_Que_value_inv Hf').
      clear Hf'.
      destruct Htmp as [d0 [ [s0 ps0] [Hf'1 Hf'2] ] ].
      destruct (instrR_value_inv Hf'1) as [Hf Hps0]; clear Hf'1.
      rename Hf'2 into Hf'.
      subst ps0.
      revert Hf Hf'.
      destruct_pose_state s.
      ...
      destruct_pose_state s1.
      intros Hf Hf'.
      red. intros Hxcal I0s Icorrs Hps Hpsleg Hpssub.
      red in Ievalgetx.
      assert (Htmp := Ievalgetx _ _ _ Hf).
      unfold Inv_EvalGet, snd, fst in Htmp.
      destruct Htmp as [I0s0 [I1s0 [Htmp [_ [Hstabvs0 [Isigs0 [Isiginfls0 [Icorrs0 [Hxinfl Hmons] ] ] ] ] ] ] ] ]; auto.
      assert (Hxcals0 : is_called x s0) by now eapply Htmp.
      assert (e : d0 = getval s0 v) by now apply Htmp.
      clear Htmp.
      ...
      red in Iwrap1.
      destruct Iwrap1 as [I0s1 [I1s1 (*[Hincl*) (*[H0ps1*) (*[H1ps1*) [H2ps1 [Icorrs1 [Isigs1 [Isiginfls1 [Hlegps1 [_ [Hsubps1 [H Hmons0] ] ] ] ] ] ] (*]*) (*]*) (*]*) ] ] ]; auto.
      assert (Hsol01 : VS.Subset (get_solved s0) (get_solved s1))
        by (clear - I1s1; simpl in *; now fsetdec).
      split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split(*; [| split; [| split] ]*) ] ] ] ] ] ] ] ] ]; auto; try easy; eauto.
      * intros d1 e1; inversion e1; subst d1; clear e1.
        ...
        now assert (Htmp := H _ eq_refl); auto.
    + assert (Htmp := wrap_rel_ErrorT_State_Que_error_inv Hf').
      clear Hf'.
      destruct Htmp as [Hf' | [d0 [ [s0 ps0] [Hf'1 Hf'2] ] ] ].
      * destruct (instrR_error_inv Hf') as [Hf Hps0]; clear Hf'.
        subst ps'.
        revert Hf.
        destruct_pose_state s.
        destruct_pose_state s1.
        ...
        red. intros Hxcal I0s Icorrs Hps Hpsleg Hpssub.
        red in Ievalgetx.
        assert (Htmp := Ievalgetx _ _ _ Hf); clear Ievalgetx.
        unfold Inv_EvalGet, snd, fst in Htmp.
        assert (H : forall p, In p ps ->
                              D.Leq (snd p) (getval s1 (fst p))).
        { intros [z u] H.
          assert (Htmp1 : Inv_sigma s s1) by intuition.
          unfold snd, fst.
          apply D.LeqTrans with (y:=getval s z);
            [now firstorder | clear - Htmp1; now firstorder]. }
        now intuition.
      * idtac.
        idtac.
        destruct (instrR_value_inv Hf'1) as [Hf Hps0]; clear Hf'1.
        ...
        assert (e : d0 = getval s0 v) by now apply Htmp.
        clear Htmp.
        assert (Iwrap1 := IH _ _ _ Evalgetx Ievalgetx Hf'); clear IH.
        assert (Hcons : forall p,
                        In p (ps ++ [(v, d0)]) ->
                        is_stable (fst p) s0 /\
                        D.Leq (snd p) (getval s0 (fst p))).
        ...
        assert (Hsubcons : subtree (rhs x) (ps ++ [(v, d0)]) = k d0)
          by (eapply subtree_step; eauto).
        red in Iwrap1.
        destruct Iwrap1 as [I0s1 [I1s1 (*[Hincl*) (*[H0ps1*) (*[H1ps1*) [H2ps1 [Icorrs1 [Isigs1 [Isiginfls1 [Hlegps1 [Htmp [Hsubps1 [H Hmons0] ] ] ] ] ] ] (*]*) (*]*) (*]*) ] ] ]; auto.
        clear H Hsubps1.
        assert (Hnxcals1 : ~ is_called x s1) by auto; clear Htmp.
        assert (Hsol01 : VS.Subset (get_solved s0) (get_solved s1))
          by (clear - I1s1; simpl in *; now fsetdec).
        split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split(*; [| split; [| split] ]*)] ] ] ] ] ] ] ] ]; auto; try easy; now eauto. }
...
- idtac.
  ...
  assert (Htmp := Iwrapx Hxcal I0s Icorrs H (legal_nil _) (subtree_nil _)); clear Iwrapx H.
  destruct Htmp as [I0s0 [I1ss0 (*[_ [_*) (*[ Hvlist1*) [Hvlist2 [Icorrs0 [Isigs0 [Isiginfls0 [Hleg [Herr [Hsub [H Hmons] ] ] ] ] ] ] (*]*) (*] ]*) ] ] ].
  split; [| split; [| split; [| split; [| split;
    [| split; [| split; [| split; [| split; [| split] ] ] ] ] ] ] ] ]; auto.
  + intros d0 e; subst d; rename d0 into d.
    assert (Htmp : forall p : Var.t * D.t, In p [] -> In x (get_infl s (fst p))) by firstorder.
    assert (H0 := H _ eq_refl (valid_nil _) Htmp); clear H Htmp; rename H0 into H.
  ...
  + split; auto.
    intros d0 e; subst d; rename d0 into d.
    assert (Hval : valid (getval s0) vlist) by now eapply H.
...
- idtac. intros x s s0 vlist.
  ...
  assert (Hxcals' : is_called x s') by auto.
  assert (H := Ievalrhs Hxcals' I0s' Icorrs'); clear Ievalrhs.
  destruct H as [I0s0 [I1s0 [Icorrs0 [Isigs' [Isiginfls' [H [Hleg [Htmp [_ [_ Hmon] ] ] ] ] ] ] ] ] ].
  assert (Hncalxs0 : ~ is_called x s0) by auto; clear Htmp.
  assert (Htmp : getval s' = getval s) by auto.
  rewrite Htmp in Hmon.
  split; [| split; [| split; [| split; [| split; [| split ] ] ] ] ]; auto; try easy; eauto.
...
- idtac. intros x d s s0 vlist.
  ...
  destruct H as [Hvlist [Hleg [_ [_ [Htmp Hmon] ] ] ] ].
  assert (H := Htmp _ eq_refl); clear Htmp.
...
- idtac. intros x d s s0 s1 s2 vlist work.
  ...
  assert (Hvals' : getval s' = getval s) by auto.
  rewrite_all Hvals'; clear Hvals'.
  assert (Hvals0' : getval s0' = getval s0) by auto.
  rewrite_all Hvals0'.
  ...
  + intros _.
    ...
    simpl in *. now intuition; fsetdec.
  + intros mu Hmonrhs Hsolmu Hlemu.
    ...
    * subst z.
      rewrite Hvals1.
      apply D.JoinUnique.
      split; [| now eapply Hmon].
      unfold cur. rewrite Hvals0'.
      assert (Htmp : leqF (getval s0) mu) by (apply Hmon; auto).
      clear - Htmp; now firstorder.
    * idtac. rewrite Evals1; auto.
      assert (Htmp : leqF (getval s0) mu) by (apply Hmon; auto).
      clear - Htmp; now firstorder.
