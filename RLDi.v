...
Module SolverRLDInstr (Sys : CSysJoinSemiLat)
...
(* Solve 1 *)
- idtac.
  intros x d1 s s1 l2 Hnstaxs s0 Heval Ieval s2 cur Hleq.
  red. intros s2' Hsol'.
  red in Ieval.
  inversion Hsol' as [| ? ? ? ? ? ? s1' Heval' |];
    [subst s2'; easy |  |].
  + unfold s1' in Heval'.
...
- idtac.
  ...
  + clear H Hnstaxs.
    ...
    unfold new_val, cur_val in s12.
    unfold s3 in Hwork.
    unfold s12, s11 in H2.
    rewrite <- H8, H4 in H2.
    assert (Hw : (w,s4) = (work,s8)).
    { rewrite Hwork, H2, H7.
      unfold s13, s2. now rewrite H8. }
...
destruct H as [I0 [I1 [Isig [Isiginfl [Icorr Hsol] ] ] ] ].
split; [| split].
- intros u Hu.
...
Theorem exactness : 
  ...
  forall mu w s,
    ...
    leqF (getval s) mu.
Proof.
intros Huniq Hmon mu w s Hsolmu Hsolallw.
assert (Isolveall : Inv_SolveAll w s_init s)
  by (now apply InvariantsLemma).
red in Isolveall.
assert (Htmp : forall x, In x w -> ~ is_called x s_init)
